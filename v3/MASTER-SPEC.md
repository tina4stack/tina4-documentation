...
### Docblock Inline Testing (`@tests`)

Attach test assertions directly to functions via docblock comments. Same syntax across all 4 languages.

**Syntax:**
```
@tests
  assert (args) === expected, "description"
  assert (args) !== unexpected, "description"
  assert_true (args), "description"
  assert_false (args), "description"
  assert_raises ExceptionClass (args), "description"
  assert_type string (args), "description"
```

**Operators:** `===`, `!==`, `>`, `<`, `>=`, `<=`, `contains`, `matches`

**Flow:**
1. `tina4 test` scans `src/` for files
2. Parses `@tests` blocks from docblocks/comments
3. Extracts function name, args, expected, operator
4. Calls function with args, compares result
5. Reports pass/fail per function
...
- Frond expression pre-compilation (replace regex with string ops)
