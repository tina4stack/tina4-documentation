...
PHP has always handled this correctly. The note records that Python, Ruby and
Node.js dropped the standalone form until 3.13.87 and now match.
...
Two things change for PHP templates. A test against the rendered `1` now sees
`true`. And a false value that used to render as nothing now renders the word
`false`, so a layout that relied on the blank output needs a conditional.
...
**Breaking in 3.13.87.** PHP used to return raw JSON from this filter while the
other three escaped it. PHP now escapes it too. Grep your templates for
`json_encode }}` and append `| raw` wherever the output lands inside a `<script>`
block. An attribute needs no change: escaped is what it always should have been.
