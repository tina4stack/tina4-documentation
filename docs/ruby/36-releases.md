...
## v3.13.137 (2026-09-23) - Gemini joins the Ai client

Google Gemini becomes a first-class Ai provider, and the v2 upgrade is pinned against a
replay other frameworks had to fix.
...
**The v2 to v3 migration upgrade is proven quiet.** When an old v2 migration table carries a
file extension in its recorded history, the upgrade must not read those rows as unapplied and
replay them. Ruby was already correct here, and a spec now locks that in so it stays correct,
matching the fixes the Python and Node upgrades needed.
...
designs your brand and UI, and ships two framework fixes underneath.
...
Underneath the skills, two fixes:
...
**The zero-dependency promise is a test now, not a claim.** The gem's runtime dependencies
stay within the pinned baseline: core Tina4 for Ruby names no third-party runtime dependency.
A new spec checks the gemspec against that baseline, so a stray dependency can never quietly
...
toolbar asks RubyGems for the latest version. On any failure - offline, a timeout, a
blocked route - it used to fall back to reporting the running version as the latest, and
the toolbar drew that as a green "You are up to date!". A developer several releases behind,
...
leading zero byte stripped, so roughly one VAPID key in 300 came out a byte short - the
private scalar a byte short of 32, which the app's own validation then rejected. The VAPID
private key is now padded to its fixed 32-byte width. The public point and the ECDH secret
...
runtime returns native result envelopes and fails closed when push is configured
without a usable provider. The developer skills for Ruby, Python, PHP, Node.js,
and tina4-js now include the Web Push API and configuration guidance.
...
- MCP `migration_create` returns `{created, resolution, ok}`, and the dev-admin `POST /__dev/api/scaffold/run` endpoint no longer inlines its own file writes for the four scaffold verbs (route, model, migration, middleware) - it shells to the CLI, which emits the envelope.
...
- **`resolution.edit_hints[]`** - every generator template now carries
  ...
  ```
  Edit these lines:
    src/orm/order.rb:14   add fields here
    src/orm/order.rb:16   add relationships here
  ```
...
- **`resolution.next[]`** - a short list of actionable next steps the
  ...
  ```
  Next:
    1. Edit src/orm/order.rb to add fields beyond the default 'name'
    2. Apply the migration:  tina4 migrate
    3. Try it:               tina4 serve  ->  curl http://localhost:7147/orders
...
All 15 verbs' templates carry markers. The Ruby scanner currently covers Ruby and SQL templates; Twig markers are baked into the `form` and `view` templates but not yet lifted into `edit_hints[]` (Wave 2 in a follow-up release).
...
**ServiceRunner.stop calls the registered Tina4::Service instance.** Before
...
Parity with tina4-python #118, tina4-nodejs #58, and the tina4-php port
landing in the same version.

**Version-contract test hardening** (`spec/version_contract_spec.rb`,
`spec/commands_manifest_spec.rb`):

- `VersionContractProbe.parse_cli_manifest(stdout, context)` locates the
  first `{` in the child's stdout before `JSON.parse`, and raises
...
- Both `JSON.parse(cli_out)` call sites routed through the resilient
  parser.
...
route param, `req`/`request`, or `res`/`response`. This is a Node-only fix,
no Ruby impact.
...
No framework code changes in Ruby for this release.
...
`can("posts.delete")`, chained after the route with `end.role(...)`, read the
cryptographically verified `roles` and `permissions` claims from the signed token, so
there is no per-rule lambda to hand-roll. Several names in one guard are OR
(`role("admin", "editor")` admits either); chain two guards for AND. Granted permissions
may carry a wildcard on the dot boundary, so a token holding `posts.*` (or a bare `*`)
satisfies a required `posts.delete`, while the required side stays concrete. A guarded
route implies auth: no or invalid token returns 401, an authenticated caller missing the
role or permission returns 403, and the handler never runs on a miss. Roles and
permissions are independent signed claims - the core never expands a role into
permissions, and a legacy singular `role` claim is coerced to a list. Feature 138 /
ADR-0058. See [Authentication](/ruby/08-authentication).

## v3.13.106 (2026-08-20) - Shared version bump

Tina4 keeps one version across all four frameworks. 3.13.106 carried a Python-only fix
(`@websocket` callable from the package name); the Ruby runtime has no equivalent import
collision and is unchanged at this version.
