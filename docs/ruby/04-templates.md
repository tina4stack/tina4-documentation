...
Ruby was inconsistent with itself: `{{ n > 3 }}` printed `false` but `{{ flag }}`
printed nothing at all. Both now print `false`. If a template leaned on the blank
output of a bare false variable, wrap it in a conditional.
...
Ruby has always escaped here. The note records that PHP, which returned raw JSON,
was brought in line in 3.13.87.
