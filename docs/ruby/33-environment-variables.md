...
## Framework-specific (Ruby)

These variables only the Ruby framework reads.

| Variable | Default | Description |
|----------|---------|-------------|
| `RACK_ENV` | _(inherits `TINA4_ENV`)_ | Rack-standard environment label. Used in env precedence when `TINA4_ENV` is unset. |
| `RUBY_ENV` | _(inherits `TINA4_ENV`)_ | Ruby-ecosystem environment label. Used when `TINA4_ENV` and `RACK_ENV` are unset. |
| `ENVIRONMENT` | _(none)_ | Selects the dotenv file to load (`.env.<ENVIRONMENT>`). |
| `TINA4_SHUTDOWN_TIMEOUT` | `10` | Seconds the supervisor waits for graceful shutdown before terminating workers. |
| `TINA4_SESSION_VALKEY_TTL` | `86400` | Per-key Valkey session expiry in seconds. |
| `TINA4_SWAGGER_CONTACT_TEAM` | _(none)_ | OpenAPI contact-team rendered in the spec. |
| `TINA4_SWAGGER_CONTACT_URL` | _(none)_ | OpenAPI contact-url rendered in the spec. |
| `PROJECT_NAME` | _(inherits `TINA4_SWAGGER_TITLE`)_ | Alternative OpenAPI title source when `TINA4_SWAGGER_TITLE` is unset. |
