---
# https://vitepress.dev/reference/default-theme-home-page
layout: home

hero:
  name: "Tina4"
  text: "Documentation"
  tagline: This is not a framework
  image: 
    src: './logo.svg'
  actions:
    - theme: brand
      text: Get Started
      link: get-started.md
    - theme: alt
      text: Python
      link: /python/index.md
    - theme: alt
      text: PHP
      link: /php/index.md


features:
    - title: Lightning-Fast ASGI Routing
      details: Effortless Python ASGI routing for high-performance web apps, supporting any server like Uvicorn—build scalable APIs in minutes.
    - title: Seamless WebSocket Integration
      details: Real-time communication made simple with built-in WebSocket support in this Python framework, perfect for chat apps and live updates.
    - title: Powerful Twig Templating
      details: Render dynamic HTML securely using Twig engine in Python, combining ease of Flask's Jinja with advanced features for full-stack development.
    - title: One-Line CRUD Generation
      details: Auto-generate CRUD interfaces and database migrations for SQLite, PostgreSQL, or MySQL—Python ORM simplicity without Django's complexity.
    - title: Instant Swagger API Docs
      details: Automatically document your Python APIs with Swagger at /swagger, enhancing developer experience like FastAPI for quick onboarding.
    - title: Hot-Reloading Development
      details: Accelerate Python web dev with Jurigged hot-reloading—edit code and see changes instantly, boosting productivity for modern apps.
    - title: Full Async Capabilities
      details: Leverage async/await in Python for efficient I/O, ideal for high-concurrency apps comparable to Starlette but with minimal setup.
    - title: Built-in JWT Authentication
      details: Secure your Python web apps with JWT, sessions, and middleware—easy authentication like Express.js, tailored for robust security.
---

