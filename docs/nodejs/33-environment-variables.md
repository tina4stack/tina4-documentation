...
## Framework-specific (Node.js)

These variables only the Node.js framework reads.

| Variable | Default | Description |
|----------|---------|-------------|
| `NODE_ENV` | _(none)_ | Node runtime convention. Gates the dev-mailbox production check. |
| `TINA4_PRODUCTION` | _(none)_ | Forces production-mode startup (cluster mode, debug overlays disabled). Set automatically by `tina4 serve --production`. Other frameworks use `TINA4_ENV=production`. |
| `TINA4_JWT_ALGORITHM` | `HS256` | JWT signing algorithm. Supports `HS256`, `HS384`, `HS512`. |
| `TINA4_SESSION_REDIS_URL` | _(none)_ | Full `redis://` URL for Redis sessions. Overrides host/port when set. |
| `TINA4_SESSION_REDIS_PREFIX` | `tina4:session:` | Key prefix for Redis session entries. |
| `TINA4_SESSION_MONGO_URI` | _(none)_ | Full MongoDB connection string for Mongo sessions (host/port style). |
| `TINA4_SESSION_MONGO_HOST` | _(none)_ | MongoDB session host. |
| `TINA4_SESSION_MONGO_PORT` | _(none)_ | MongoDB session port. |
| `TINA4_SESSION_MONGO_USERNAME` | _(none)_ | MongoDB session username. |
| `TINA4_SESSION_MONGO_PASSWORD` | _(none)_ | MongoDB session password. |
| `TINA4_WS_PORT` | `8080` | Default standalone WebSocket server port when none is passed to the constructor. |
| `TINA4_WS_MAX_BACKLOG` | `1048576` | Socket write-backlog drop threshold for slow clients (bytes). Distinct from frame size. |
