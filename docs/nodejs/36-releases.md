...
pointed at a document that does not exist, so a request that missed every route fell through to
a docs page that never finished loading. It now points at a document that exists, and a test
proves the gate that decides when to serve the UI agrees across the boot path.

**The zero-dependency promise is a test now, not a claim.** Importing the eager core barrel
pulls in no third-party runtime dependency. A new test imports the barrel and fails if
anything outside Node's standard library rides along, so a stray import can never quietly
...
without a usable provider. The developer skills for Node.js, Python, PHP, Ruby,
...
- **`resolution.edit_hints[]`** - every generator template now carries
  ...
  ```
  Edit these lines:
    src/orm/Order.ts:14   add fields here
    src/orm/Order.ts:16   add relationships here
  ```
...
- **`resolution.next[]`** - a short list of actionable next steps the
  ...
  ```
  Next:
    1. Edit src/orm/Order.ts to add fields beyond the default 'name'
    2. Apply the migration:  tina4 migrate
    3. Try it:               tina4 serve  ->  curl http://localhost:7148/orders
...
The 10 TS/JS-emitting verbs carry markers in their templates; `form`, `view`, and `migration` emit `.twig` / `.sql` files and legally return `edit_hints: []` (Wave 2 extends the scanner to twig/SQL for full parity).
...
The dev toolbar renders styled and functional under the framework's own default `default-src 'self'` CSP. Its CSS and JS move out of the injected HTML into two external routes, `/__dev/toolbar.css` and `/__dev/toolbar.js`, every event is wired with `addEventListener`, and the live reloader is gated on a `data-reload` attribute that the AI port sets to `0`. Node's toolbar previously emitted inline `style=`, `onclick=`, and `<script>`, which the strict default CSP silently blocked; it is now CSP-clean like the other three, locked by the shared `devadmin-toolbar-csp-clean` contract invariant. Issue #115.

## v3.13.111 (2026-08-21) - Graph database layer

A `GraphDatabase` layer shaped exactly like the relational `Database`. `GraphDatabase.create(url)` and `GraphDatabase.fromEnv()` pick the engine from the URL scheme, and one portable surface (`addNode`, `addEdge`, `getNode`, `updateNode`, `deleteNode`, `neighbors`, `traverse`) works identically on every engine, while `query` and `execute` pass native GQL, Cypher, or AQL straight through. Proven live against Ultipa (GQL), Neo4j and Memgraph (one Bolt/Cypher adapter), and ArangoDB (AQL). The engine driver is an optional dependency (`tina4-ultipa`, `neo4j-driver`, `arangojs` in `optionalDependencies`), so the core stays zero-dependency. Configure with `TINA4_GRAPH_URL` and `TINA4_GRAPH_CONNECT_TIMEOUT`. Feature 139 / ADR-0059. See [Graph databases](/nodejs/graph-databases).
...
The response cache never serves one session's authenticated response to another caller: it keys on the session, so a per-user response cannot leak across callers (#52).

## v3.13.108 (2026-08-20) - Lockfile sync and dev-toolbar gzip fix

`npm ci` works again: the lockfile is synced with the optional `pg` and `@redis/client` dependency trees (#51). And the dev toolbar no longer corrupts an already-gzipped HTML response.

## v3.13.107 (2026-08-20) - RBAC: role and permission guards

Authorization arrives as two chainable methods. `.role("admin")` and
`.can("posts.delete")` read the cryptographically verified `roles` and `permissions`
claims from the signed token, so there is no per-rule guard array to hand-roll. Several
names in one guard are OR (`.role("admin", "editor")` admits either); chain two guards
for AND. Granted permissions may carry a wildcard on the dot boundary, so a token
holding `posts.*` (or a bare `*`) satisfies a required `posts.delete`, while the
required side stays concrete. A guarded route implies auth: no or invalid token returns
401, an authenticated caller missing the role or permission returns 403, and the handler
never runs on a miss. Roles and permissions are independent signed claims - the core
never expands a role into permissions, and a legacy singular `role` claim is coerced to
a list. Feature 138 / ADR-0058. See [Authentication](/nodejs/08-authentication).
...
(`@websocket` callable from the package name); the Node runtime has no equivalent import
