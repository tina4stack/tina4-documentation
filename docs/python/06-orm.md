...
## Connecting Models to a Database

A model needs a database before any operation runs. Call `save()`, `find_by_id()`, `where()` — anything that touches data — before a database is bound and the ORM raises:

```
RuntimeError: No database bound. Call bind_database(db) or set DATABASE_URL in .env
```

There are two ways to give the ORM a database. Pick one.

### Option A — Auto-discovery via `TINA4_DATABASE_URL`

The simplest path. Set the URL in `.env` and the ORM builds and binds the connection itself the first time a model is used:

```bash
# .env
TINA4_DATABASE_URL=postgresql://user:pass@localhost:5432/mydb
```

No code required. This is the right choice for a single, unnamed connection. Note the variable name is `TINA4_DATABASE_URL` — a plain `DATABASE_URL` is **not** picked up by auto-discovery.

### Option B — Explicit binding with `bind_database()`

When you need control — your own URL variable, a connection pool, or more than one database — create the `Database` yourself and bind it once at startup. The natural place is your application package's `__init__.py`, which is imported before any route or model:
...
# The global default — every model without an explicit _db uses this.
bind_database(Database("postgresql://user:pass@localhost:5432/mydb"))
```

`bind_database(db)` sets the default connection for **all** models. That single call is what stands between you and the `No database bound` error.

### Named connections (read replicas, multiple databases)

Pass `name=` to register a connection under a name, then point a model at it with `_db`:

```python
# src/__init__.py
bind_database(Database(PRIMARY_URL))                 # default (read/write)
bind_database(Database(REPLICA_URL), name="read")    # named connection
```

```python
class Report(ORM):
    _db = "read"          # this model uses the "read" connection
    table_name = "reports"
```

Models without a `_db` use the default binding; models with `_db = "read"` use the named one. This is how you split reads onto a replica while writes go to the primary.

> **Why binding is required.** The ORM never guesses a connection. It resolves, in order: a per-model `_db`, the global default set by `bind_database(db)`, then auto-discovery from `TINA4_DATABASE_URL`. If none of those is present, it raises rather than silently writing to the wrong place.
