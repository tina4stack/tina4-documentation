...
# in a route
from src.app.user_service import current_user
...
**Background tasks carry no session.** A task on a timer serves no request and holds no cookie, so it has no current user to read. Hand the task the identity it needs when you enqueue it, the user id or the session id, never a global lookup.
...
@get("/api/report")
async def report(request, response):
    user = Auth.authenticate_request(request.headers)   # validates the Bearer JWT / API key
    if user is None:
