...
These variables only the Python framework reads.

| Variable | Default | Description |
|----------|---------|-------------|
| `TINA4_OPEN_BROWSER` | `true` | Inverse browser toggle for `tina4 serve`. Set `false` to stop the browser opening on start (mirrors `TINA4_NO_BROWSER`). |
| `TINA4_TOKEN_EXPIRES_IN` | _(inherits `TINA4_TOKEN_LIMIT`)_ | Preferred JWT-lifetime name (minutes). Falls back to `TINA4_TOKEN_LIMIT`. |
