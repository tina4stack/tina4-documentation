...
| `TINA4_LOG_OUTPUT` | `stdout` | Where logs go. Options: `stdout`, `file`, `both`. When unset, the log file is written only in development (`TINA4_DEBUG`); production / containers are stdout-only (no file, to avoid image-layer/disk bloat). Explicit `file`/`both` always writes a file. |
