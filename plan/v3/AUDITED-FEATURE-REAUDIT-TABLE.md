...
Features 39, 40, 47, 50, 55, 64 and 79 have evidence elsewhere in bundled
audits, the contract map or later work, but the audit file does not include them
in its stated 32. They are listed separately below rather than silently changing
the audit's denominator. There is no evidence-backed route from 32 to 44 in the
audit file.
...
| 11 | Rate limiter | Closed only inside 11/12/79 bundle | **Standalone audit complete 2026-08-10; owner decisions pending** | `011-rate-limiting.md` has 15 findings, shared-fixture design and a future-language formula |
| 12 | Response types | Closed only inside 11/12/79 bundle | Re-audit in progress | `012-response-types.md` exists; standalone contract is being rebuilt |
...
| 28 | Frond lexer | Closed only inside 28-31 bundle | Reopened / queued | `028-frond-lexer.md` exists |
| 29 | Frond parser | Closed only inside 28-31 bundle | Reopened / queued | `029-frond-parser.md` exists |
| 30 | Frond compiler | Closed only inside 28-31 bundle | Reopened / queued | `030-frond-compiler.md` exists |
| 31 | Frond runtime | Closed only inside 28-31 bundle | Reopened / queued | `031-frond-runtime.md` exists |
...
| 41 | JWT authentication | Closed only inside 41/42 bundle | Reopened / queued | `041-jwt-authentication.md` exists |
| 42 | Session handling | Closed only inside 41/42 bundle | Reopened / queued; 42.1-42.6 packets owed | `042-session-handling.md` exists |
...
| ---: | --- | --- | --- |
| 39 | Template caching | Historical evidence mixed into Feature 43 | Re-audit in numeric order; `039-template-caching.md` exists |
| 40 | Fragment caching | Historical evidence mixed into Feature 43 | Re-audit in numeric order; `040-fragment-caching.md` exists |
| 47 | Swagger / OpenAPI | Layer-2 contract map, 10 invariants | Re-audit in numeric order; `047-swagger-openapi.md` exists |
| 50 | HTTP client | Later surface-parity work, no fixture | Re-audit in numeric order; `050-http-client.md` exists |
...
| 64 | `tina4 routes` CLI | Historical evidence hidden in the 11/12/79 bundle | Re-audit in numeric order; `064-routes-cli.md` exists |
| 79 | Route groups | Historical evidence hidden in the 11/12/79 bundle | Re-audit in numeric order; `079-route-groups.md` exists |
...
| Individual language-port packets complete | **No** | Top-level grouped files are now split, but each packet still needs the full 3.14 capsule |
