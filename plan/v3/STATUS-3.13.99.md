...
**RELEASE CUT (owner 2026-08-13): 3.13.99 = Phases 1-5 only. Phase 6 Frond (48/49, 52, 54-60) DEFERRED to 3.13.100 fast-follow, alongside the already-deferred feature 50 Frond compiler.** Remaining before the 3.13.99 merge+tag: (1) two real pre-existing bugs found during Phase-5 verification - PHP Router literal-paren-path 404 + PHP built-in-Server first-time session-cookie not emitted (both cross-checked all four); (2) **close the 16 OWED conformance invariants with real per-language runners in all four (owner decision 2026-08-13: "close both, then tag" - zero owed at the tag): `logger_contract.json` (8, the structured Logger - genuine cross-framework gap, "59 cases not wired") + `adapter_contract.json` (8, ADR-0044 14-method adapter interface - behaviour already proven four-way via the 5 provider fixtures, but the granular structural spec gets its own runner too)**; (4) pre-merge test hygiene (PHP/Ruby stale request-id regex, PHP DevAdmin risky-handler, PHP 7146->7145 stray refs; Python overlay test already fixed d619af0). Then merge feature/release3.13.99 -> v3, full-suite green at merged HEAD on the lab (auditor 266/266 proven / 0 owed), PAUSE for owner go-ahead, tag.
