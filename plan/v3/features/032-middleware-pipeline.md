# Feature 032: Middleware pipeline

## Identity and status

- Matrix identity: 32 — Middleware pipeline
...
Feature 32 is **not stable**. The four approved characterisation suites are
...
- route matching, auth-gate placement and 404/405 selection to Feature 30;
...
Feature 32 still owns whether the delegated policy is invoked at the correct
...
Feature 32 owns one resolver API and exact failure behavior, not the policy list
...
`CORS`; PHP, Ruby and Node only wire `ResponseCache`. Feature 32 must not paper
...
routes (Feature 37 health/readiness) are selected in the Feature 30 system tier
...
retaining the Feature 30 auth boundary:
...
this document the Feature 32 implementation contract.
...
These close the DESIGN half of the FINAL bar for Feature 32. Remaining to reach
...
| --- | --- |
| Feature 30 dispatch | carry one entered stack through match, auth, fallbacks and finalization |
...
5. Adapt class hooks and continuation functions to one nested execution model
   while retaining the Feature 30 auth boundary.
...
A clean-room language port implements Feature 32 in this order:
...
Feature 32 is **audit-complete and decision-ready**, not implementation-complete
...
conform before Feature 32 can be called complete.
