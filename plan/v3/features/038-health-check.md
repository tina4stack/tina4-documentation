# Feature 038: Health and readiness endpoints

## Identity and status

- Matrix identity: 38 — Health and readiness endpoints
- Audit state: decision-ready
- Audit note: Implementation is deliberately deferred
- Dependencies: Feature 1 dotenv, Feature 31 dispatch and Feature 33 middleware
...
Feature 38 is **not complete**. All four focused suites pass and all four ports
...
- ordinary route matching and HTTP method semantics to Feature 31;
- application middleware behavior to Feature 33;
...
- graceful process termination to Feature 39.

The adapter features own a cheap, non-mutating dependency probe. Feature 38 owns
...
origin only wires the approved Feature 1 fixture and does not change Feature 38.
...
- `GET` and `HEAD` are supported. `HEAD` has the same status and headers as
  `GET` and no body.
- `OPTIONS` follows the Feature 31 automatic method contract.
...
Both configured paths use the Feature 31 exact literal system-path validator:
...
Feature 38 carried its decisions in the prose rather than a decisions section. The
...
This is intentionally stronger than ordinary Feature 31 specificity. A probe
...
1. Implement Feature 1 loading and Feature 31 literal-path validation first.
2. Create a system-route registry that is dispatched before application routes
   and Feature 33 middleware.
