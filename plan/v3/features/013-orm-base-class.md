# Feature 13: ORM base class (SQL-first)
...
iteration inline. This is the same disease as feature 6's dispatch and feature 2's
...
locking in with tests before any structural work starts, exactly as feature 6 does.
...
Same discipline as feature 6, and it depends on feature 3 landing first because the
...
1. **Lock the behaviour first.** The D1 contract is currently correct in all four
   ...
   get it GREEN before touching structure. Same safety argument as feature 6.
...
## Note for feature 14 (soft delete)

`01-FEATURE-MATRIX.md` names feature 14 "Soft delete (**deleted_at**, restore,
...
So the feature matrix itself carries a stale name for the row. Fix it when feature
14 is audited; recorded here so it is not lost.
