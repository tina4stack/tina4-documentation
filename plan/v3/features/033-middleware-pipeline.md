# Feature 033: Middleware pipeline

## Identity and status

- Matrix identity: 33 — Middleware pipeline
...
- Existing ADRs: ADR-0012, ADR-0014 (result-table clause superseded by ADR-0045)
- Shared fixtures: none

- Release boundary: v3 / 3.14.0; parity-breaking corrections are permitted
- Re-audit date: 2026-08-10

Feature 33 is **not stable**. The four approved characterisation suites are
...
- route matching, auth-gate placement and 404/405 selection to Feature 31;
...
Feature 33 still owns whether the delegated policy is invoked at the correct
...
Feature 33 owns one resolver API and exact failure behavior, not the policy list
...
`CORS`; PHP, Ruby and Node only wire `ResponseCache`. Feature 33 must not paper
...
routes (Feature 38 health/readiness) are selected in the Feature 31 system tier
...
retaining the Feature 31 auth boundary:
...
this document the Feature 33 implementation contract.
...
These close the DESIGN half of the FINAL bar for Feature 33. Remaining to reach
...
| --- | --- |
| Feature 31 dispatch | carry one entered stack through match, auth, fallbacks and finalization |
...
5. Adapt class hooks and continuation functions to one nested execution model
   while retaining the Feature 31 auth boundary.
...
A clean-room language port implements Feature 33 in this order:
...
Feature 33 is **audit-complete and decision-ready**, not implementation-complete
...
conform before Feature 33 can be called complete.
