...
| --- | --- | --- |
| MCP-02 | SECURITY (HIGH), FIX PYTHON: the tool-invocation REST shim `POST /__dev/api/mcp/call` is UNGATED in the Python master (`dev_admin/__init__.py:2976` - self-verified). It runs any tool (incl. `database_execute`, `file_write`) with no `_mcp_request_allowed` check, while every sibling surface IS gated and PHP/Ruby/Node all gate their `/call`. On a `TINA4_DEBUG=true` server bound to `0.0.0.0`, a remote unauthenticated caller can invoke every tool. Shipped on v3. Untested (no test drives `/call`; all gate tests use loopback). | Python adds the same gate the sibling handlers use to the top of `_api_mcp_call` (`if not _mcp_request_allowed(request): return response(..., 404)`), plus a red-before-green regression test (remote no-token -> 404, tool did not run; remote + valid token -> 200) added to all four (Python fails-then-passes; the three ports pass immediately, locking their correct behaviour). A fix task is spawned. FIX PYTHON. |
...
| MCP-04 | Test coverage: the authorization gate (remote-denied / token-required / XFF-ignored) is unit-proven but NOT driven end-to-end over the wire in Python and Node - their endpoint tests only toggle the CAPABILITY gate from loopback, and Node's token check has no test at all. PHP and Ruby DO have the wire-level negatives (real remote peer + spoofed-XFF -> 404). This gap is exactly why MCP-02 survived in Python. | Python and Node add wire-level gate negatives: a real request from a non-loopback peer is 404'd without a token, 200 with a valid token, and a spoofed `X-Forwarded-For: 127.0.0.1` is ignored - on every surface INCLUDING `/call`. |
