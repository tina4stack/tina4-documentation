# Feature 035: Rate limiting

## Identity and status

- Matrix identity: 35 — Rate limiting
- Audit state: decision-ready
- Audit note: Implementation is deliberately deferred
- Dependencies: Feature 1 dotenv, Feature 2 logging, Feature 31 dispatch,
  Feature 33 middleware, Feature 38 health/readiness and Feature 34 CORS
...
Feature 35 is **not complete**. The old audit repaired the untrusted forwarded-
...
- canonical request/header representation and route matching to Feature 31;
- guaranteed final unwinding to Feature 33;
- health/readiness path aliases to Feature 38;
- valid CORS-preflight classification and final CORS headers to Feature 34;
- response JSON serialization and HEAD body suppression to Feature 30;
...
commit ahead of the public lab head and contains no Feature 35 implementation
...
user-agent, path, token or authenticated-user specific in Feature 35.
...
elif Feature 34 says valid CORS preflight:
    continue without consuming or adding rate headers
else:
...
The Feature 38 canonical paths and their approved compatibility aliases bypass
...
decision is stored on Feature 31's dispatch context. Feature 33's final unwind,
...
- `close()` releases timers/connections idempotently through Feature 39;
...
headers survive. Feature 34 adds final CORS headers to an allowed Origin even
on 429. Feature 30 suppresses the body for HEAD while preserving status and
...
of the FINAL bar for Feature 35 (default limit 240/60, confirmed 2026-08-10).
...
| Node | Stop unconditional default registration; make function/class/middleware delegate to one coordinator; validate instead of propagating `NaN`; bypass Feature 38 probes/preflight; unify response/cleanup and support the shared provider contract. |
...
Feature 35 may move from decision-ready to final only when:
...
- idle buckets/timers/connections are bounded and close through Feature 39;
...
Feature 35 has a sound common core - per-IP limiting, trusted-peer gating and
