# Feature 039: Graceful shutdown

## Identity and status

- Matrix identity: 39 — Graceful shutdown
- Audit state: decision-ready
- Audit note: Implementation is deliberately deferred
- Dependencies: Feature 1 dotenv, Feature 38 health/readiness and the active
...
- Existing ADRs: ADR-0017 (state-machine/deadline/validation/hook/worker clauses superseded by ADR-0047)
- Shared fixtures: none

- Release boundary: v3 / 3.14.0; parity-breaking corrections are permitted
- Re-audit date: 2026-08-10

Feature 39 is **not complete**. The original audit repaired serious
...
- path/status behavior of liveness and readiness to Feature 38;
- HTTP dispatch and response completion to Feature 31;
...
Those features must register resources or in-flight work with Feature 39; they
...
Feature 39 regression. Existing server issues concern separate CLI/PHP-FPM and
...
1. Atomically enter `QUIESCING`, capture the monotonic start/deadline and log
   the reason.
2. Mark Feature 38 readiness unavailable. If an already accepted connection can
...
- **Granian:** either map a real drain deadline and lifecycle hooks or fail
  startup when selected with an unsupported Feature 39 contract. A warning that
...
selects must meet Feature 39 or fail selection explicitly.
...
| H9-03 | P1 | Long-running queue CLI paths do not share Feature 39: Python catches Ctrl-C, PHP/Node register SIGINT only, Ruby has no graceful coordinator. Container SIGTERM can interrupt a claimed job and skip backend cleanup. | reuse one coordinator and prove current-job ack/redelivery against real backends |
...
Feature 39 carried its decisions in the prose. The review surfaced three; Andre
settled them.
...
Feature 38 startup/liveness/readiness probes remain separate. Kubernetes already
...
Feature 39 is complete only when:
