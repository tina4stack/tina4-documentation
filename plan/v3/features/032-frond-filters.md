...
surface. Backed by the byte-identical 82-case expression corpus (feature 28-31), filter
...
So a filter name is the same class of thing as feature 18's JSON keys: **data, and data
...
3. **The filter registry is data, not control flow.** This is feature 28-31's finding
...
| extensibility | `add_filter_registers_a_working_custom_filter` | `add_filter_does_not_silently_replace_a_builtin` (feature 36's boundary) |
