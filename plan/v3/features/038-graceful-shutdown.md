# Feature 038: Graceful shutdown

## Identity and status

- Matrix identity: 38 — Graceful shutdown
- Audit state: decision-ready
- Audit note: Implementation is deliberately deferred
- Dependencies: Feature 1 dotenv, Feature 37 health/readiness and the active
...
Feature 38 is **not complete**. The original audit repaired serious
...
- path/status behavior of liveness and readiness to Feature 37;
- HTTP dispatch and response completion to Feature 30;
...
Those features must register resources or in-flight work with Feature 38; they
...
Feature 38 regression. Existing server issues concern separate CLI/PHP-FPM and
...
1. Atomically enter `QUIESCING`, capture the monotonic start/deadline and log
   the reason.
2. Mark Feature 37 readiness unavailable. If an already accepted connection can
...
- **Granian:** either map a real drain deadline and lifecycle hooks or fail
  startup when selected with an unsupported Feature 38 contract. A warning that
...
selects must meet Feature 38 or fail selection explicitly.
...
| H9-03 | P1 | Long-running queue CLI paths do not share Feature 38: Python catches Ctrl-C, PHP/Node register SIGINT only, Ruby has no graceful coordinator. Container SIGTERM can interrupt a claimed job and skip backend cleanup. | reuse one coordinator and prove current-job ack/redelivery against real backends |
...
Feature 38 carried its decisions in the prose. The review surfaced three; Andre
settled them.
...
Feature 37 startup/liveness/readiness probes remain separate. Kubernetes already
...
Feature 38 is complete only when:
