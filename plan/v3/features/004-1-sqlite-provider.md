# Feature 4.1: SQLite provider

## Identity and status

- Parent: Feature 4 database write facade and provider conformance.
...
capabilities. Feature 4 owns CRUD composition. Feature 5 owns URL parsing.
...
- An absolute filesystem path is not prefixed or rewritten. A relative path is
  resolved by Feature 5 before the provider receives it.
...
4. Compile null map filters in the Feature 4 builder as `IS NULL`.
...
operations idempotent. Run the shared Feature 4 and SQLite fixtures against real
...
- [ ] Parent Feature 4 owner decisions approved or amended.
