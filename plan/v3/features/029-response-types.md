# Feature 029: HTTP response model and representation types

## Identity and status

- Matrix identity: 29 — HTTP response model and representation types
- Audit state: decision-ready
- Audit note: Implementation is deliberately deferred
- Dependencies: Feature 28 request model, Feature 30 dispatch, Feature 32
...
- Required follow-up decision: ADR-0050, superseding the Feature 29 clauses of
  ADR-0019 rather than silently rewriting them
- Re-audit date: 2026-08-10

Feature 29 is **not complete**. Its 284 focused local/lab checks remain green,
...
- handler return interpretation, HEAD fallback and final socket commit to
  Feature 30;
- after hooks and guaranteed final unwind to Feature 32;
...
Feature 29 still specifies how those features compose at commit: there is one
...
response documentation. No open issue tracks the Feature 29 re-audit contract.
...
| H12-17 | documentation | central | Historical and master material claims status/media/file parity that current source contradicts, and the old bundle hid Features 29, 114 and 31 under one filename. |
...
1. The response builder is buffered. Helpers select representation state and
   return the same response object; they do not write to the socket. Feature 30's
   finalizer commits once after Feature 32 unwinds.
...
8. Connect the builder to Feature 30's one finalizer after Feature 32 unwind.
   Apply HEAD/bodyless rules there, then commit once.
9. Register file/stream cleanup with disconnect and Feature 38 shutdown paths.
...
Feature 29 may move from decision-ready to final only when:
...
Feature 29 has recognizable helpers in all four languages, but names are not
